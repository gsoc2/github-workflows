...
Describe 'Invoke-Gsoc2Server' {
    It "works fine with a simple callback" {
        $result = Invoke-Gsoc2Server {
            Param([string]$url)
    ...
    It "rethrows an exception and recovers" {
        { Invoke-Gsoc2Server { throw "hello there" } } | Should -Throw "hello there"
        $result = Invoke-Gsoc2Server {}
        $result.ServerStdOut | Should -Contain 'HTTP server listening on <ServerUri>'
    }
...
{"event_id":"9ec79c33ec9942ab8353589fcb2e04dc","dsn":"https://e12d836b15bb49d7bbf99e64295d995b:@gsoc2.io/42"}
