...
function RunApiServer([string] $ServerScript, [string] $Uri = $ServerUri)
{
    ...
    $out = New-Object InvokeGsoc2Result
    $out.ServerStdOut = @()
    $out.ServerStdErr = @()
...
function Invoke-Gsoc2Server([ScriptBlock] $Callback)
{
    # start the server
    $httpServer = RunApiServer "gsoc2-server"
...
Export-ModuleMember -Function Invoke-Gsoc2Server
